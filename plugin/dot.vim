...
" kawate タイトル文字の変更
"let s:DOT_REGEXP =      '^\(\.\+\)\s*\(.*\)$'
"let s:DOT_REGEXP =      '^\(\*\+\)\s*\(.*\)$'  " 2009-10-22 title charactor change '.' to '*'
"let s:DOT_REGEXP =    '^-*\(\*\+\)\s*\(.*\)$'  " 2010-09-30 タイトルの先頭にluaのコメント(--)が入ってもOKにした
"let s:DOT_REGEXP = '^[-#]*\(\*\+\)\s*\(.*\)$'  " 2011-05-10 タイトルの先頭にRubyのコメント(#)が入ってもOKにした
let s:DOT_REGEXP = '^[-#;]*\(\*\+\)\s*\(.*\)$'  " 2011-08-03 タイトルの先頭にTeraTermのコメント(;)が入ってもOKにした
"let s:DOT_REGEXP = '^\(\function\)\s*\(.*\)$'  "「function ～」でマッチ

" kawate タイトル文字の変更
" 2009-10-24 title charactor change '.' to '*'
